...
//UX-Skills Section Component with lang Prop (which is used to know which langauge the app has to load the appropriate  translated JSON data)
const UXSkills = ({ lang }) => {
  ...
  const content = require(`../../../locales/${lang}/dynamic-content/uxskills_content.json`);
      ...
      <div className="resume-block-description__wrapper">
        <Fade triggerOnce>
          <Typography className="resume-block-description__title" variant="h6">
            {useNextTranslate('resume:ui_resume-block-title-5')}
          </Typography>
        </Fade>
      </div>
      <Fade triggerOnce>
        ...
        >
          {content.map((uxskills) => (
            <div key={uxskills.uxskill_chip_id} style={{ padding: '3px' }}>
              <Chip
                className="resume-chip"
                label={uxskills.uxskill_chip_name}
              />
            </div>
          ))}
        </Stack>
